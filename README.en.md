...
The repository reserves [`assets/examples/`](assets/examples/) for future work. Only finished 017 artwork verified by the project owner will be added; no post or image from another style is used as a placeholder.

Future samples will demonstrate 017's adaptability. Their subjects, metaphors, palette, copy, and canvas ratios will never become generation references or defaults.

## Four outputs, one Korean flat logic
...
| --- | --- | --- |
| `top-bottom` | source-adaptive | complete source above, 017 naive Korean flat editorial illustration below; both panels retain the source size and split exactly 50/50 |
| `left-right` | source-adaptive | complete source left, 017 naive Korean flat editorial illustration right; both panels retain the source size and split exactly 50/50 |
...
Exact user pixels > explicit ratio or destination > source adaptation for ordinary modes. The original `017.md` used a 3:4 creative canvas, but that historical example is not a silent default in the current Skill.
...
- **Independent set:** every device references only the source photograph and may explore different rounded forms, broken-line rhythm, bright fields, whitespace, and copy relationships.
...
The image model creates rounded forms, rough broken outlines, pure flat fills, bright fields, lively asymmetry, whitespace, and integrated type. `scripts/compose_panel.py` only plans canvases, performs exact 50/50 raster composition, finalises dimensions, and audits results. It never fakes artwork with programmatic drawing.
