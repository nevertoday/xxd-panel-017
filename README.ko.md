...
| 모드 | 크기 규칙 | 결과물 |
| --- | --- | --- |
| `top-bottom` | 원본 적응형 | 위에는 전체 원본, 아래에는 017 한국식 소박한 플랫 편집 일러스트. 두 패널이 원본 크기를 유지하며 정확히 50/50 |
| `left-right` | 원본 적응형 | 왼쪽에는 전체 원본, 오른쪽에는 017 한국식 소박한 플랫 편집 일러스트. 두 패널이 원본 크기를 유지하며 정확히 50/50 |
| `design-only` | 원본 적응형 | 원본 사진은 보이지 않고 변환 디자인만 표시. 원본 비율과 크기 유지 |
| `wallpaper-pack` | 기기별 네 가지 크기 | 휴대전화, iPad, 데스크톱, 어린이용 스마트워치 PNG 개별 출력 |

정확한 사용자 픽셀 > 지정 비율 또는 용도 > 일반 모드의 원본 적응형 순서입니다. 원본 `017.md`의 3:4는 초기 창작 캔버스일 뿐 현재 Skill의 숨은 기본값이 아닙니다.
...
- **독립형 네 작품:** 각 기기는 원본 사진만 참조하며 둥근 형태, 끊긴 선의 리듬, 밝은 색면, 여백, 문구 관계를 각각 탐색합니다.
...
자동 문구는 주제, 보이는 동작, 관계, 확인된 장소, 감정 단서 또는 사진에 근거한 이야기 관계에서 짧은 제목을 추출합니다. 이 사진과 강하게 묶인 작은 공감을 만들되 이야기나 말장난을 지어내지 않습니다.
...
이미지 모델이 둥근 형태, 거칠고 끊긴 선, 순색 평면 채색, 밝은 색면, 경쾌한 비대칭, 여백과 타이포를 만듭니다. `scripts/compose_panel.py`는 캔버스 계획, 정확한 50/50 래스터 합성, 최종 크기와 검수만 수행하며 프로그램 그림으로 결과물을 위조하지 않습니다.
...
- [Skill 워크플로](SKILL.md)
- [중문 전체 프롬프트](references/xxd-panel-017-prompt.zh-CN.md)
- [영문 전체 프롬프트](references/xxd-panel-017-prompt.en.md)
