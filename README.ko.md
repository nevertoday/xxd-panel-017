<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 017 프로젝트 배너" width="1200">
</p>
...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#네-가지-출력-하나의-한국식-플랫-논리)
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 017 예시 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 017 예시 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 017 예시 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 017 예시 8"></td>
  </tr>
</table>

## 추가 3:4 상하 구성 예시

위 16:9 예시와 같은 네 개의 독립 소스를 완전한 3:4 상하 캔버스로 다시 생성했습니다. 위쪽은 현실 사진을 유지하고 아래쪽은 Panel 017의 원본 프롬프트에 따라 재구성합니다. 영어 문구는 각 사진의 내용·분위기·함의에서만 생성했으며 모든 결과를 개별 검수했습니다.

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 017 추가 상하 예시 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 017 추가 상하 예시 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 017 추가 상하 예시 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 017 추가 상하 예시 4"></td>
  </tr>
</table>
...
> 둥근 형태 · 거칠고 끊긴 선 · 순색 평면 채색 · 밝은 색면 · 경쾌한 비대칭

XXD Panel 017은 Codex와 호환 에이전트를 위한 이미지 생성 Skill입니다. 원본의 대략적인 배치, 사물 관계, 핵심 동작, 최소 세 가지 정체성 단서를 보존한 뒤 둥글고 약간 과장된 형태, 거칠게 흔들리며 일부 끊기는 검은 선, 고명도·고순도의 평면 색으로 재구성합니다.
...
**Panel 017**은 원본 사진의 핵심을 보존하면서 다음과 같은 시각 방향을 원하는 작업에 적합합니다: **사진을 밝고 친근한 한국식 소박한 플랫 편집 일러스트로**. 먼저 용도와 원하는 결과를 확인한 뒤 구성, 크기, 텍스트와 전달 형식을 결정합니다.
...
> XXD Panel 017으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.
...
## 빠른 판단: XXD Panel 017이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 사진을 밝고 친근한 한국식 소박한 플랫 편집 일러스트로 |
| **한눈에 알아볼 특징** | 둥근 형태 · 거칠고 끊긴 선 · 순색 평면 채색 · 밝은 색면 · 경쾌한 비대칭 |
...
```text
배치·관계·동작·세 가지 정체성 단서 고정 → 둥글고 약간 과장되게 단순화 → 거칠게 흔들리고 일부 끊긴 검은 선 → 고명도·고순도 평면 색 → 크기·어긋남·가림·가벼운 크롭·여백으로 하나의 중심 구성 → 제목과 작은 글자를 윤곽·색면·여백·축에 통합
```

무관한 사진으로 바꿔도 배치, 관계, 동작, 형태, 색면, 문구가 거의 그대로 성립한다면 017이 아닙니다.

## 완성작에서 바로 알아볼 수 있는 스타일 특징

- **원본 정체성:** 대략적인 배치, 사물 관계, 핵심 동작, 최소 세 가지 고유 단서를 지킵니다.
- **둥근 단순화:** 형태는 단순하고 약간 과장되며 사실적 빛, 원근, 재질은 제거하지만 한눈에 알아볼 수 있습니다.
- **거칠고 끊긴 선:** 검은 윤곽은 흔들리고 굵기가 조금 달라지며 일부 자연스럽게 끊깁니다. 매끈한 벡터나 균일한 만화선이 아닙니다.
- **순색 평면 채색:** 깨끗하고 불투명한 색면을 쓰며 깊이는 평면 겹침, 크기, 위치만으로 표현합니다.
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 017의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-017 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-017 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-017 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-017 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-017 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-017 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 017 적용, 공통 설정은 한 번만 확정
/xxd-panel-017 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 `~/.claude/skills/xxd-panel-017`에 연결할 수 있습니다. 설치 후 에이전트 세션을 다시 시작하세요.
...
- [Skill 워크플로](SKILL.md)
- [중문 런타임 어댑터](references/xxd-panel-017-prompt.zh-CN.md)
- [영문 런타임 어댑터](references/xxd-panel-017-prompt.en.md)
...
**사진을 그저 귀엽게 만들지 말고, 더 적은 형태와 더 밝은 색, 자유로운 끊긴 선으로 그 사진을 알아보세요.**
