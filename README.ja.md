<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 017 プロジェクトバナー" width="1200">
</p>

<div align="center">

# 🦁 XXD Panel 017｜無邪気グラフィック手帳

### 写真を、明るく親しみのある韓国風の素朴なフラット編集画へ

[![Codex Skill](https://img.shields.io/badge/Codex-Skill-000000?style=flat-square)](./SKILL.md)
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#4つの出力を支えるひとつの韓国風フラットロジック)
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 017 作例 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 017 作例 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 017 作例 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 017 作例 8"></td>
  </tr>
</table>

## 追加 3:4 上下構成作例

上の 16:9 作例と同じ4点の独立素材を、完全な 3:4 上下構成として再生成しました。上部は現実の写真を保ち、下部は Panel 017 の原文プロンプトに従って再構成しています。英語コピーは各写真の内容・空気・含意だけから生成し、全点を個別に確認しました。

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 017 追加上下作例 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 017 追加上下作例 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 017 追加上下作例 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 017 追加上下作例 4"></td>
  </tr>
</table>
...
> 丸い形 · 粗く途切れる線 · 純色の平塗り · 明るい色面 · 軽快な非対称

XXD Panel 017 は、Codex と互換 Agent のための画像生成 Skill です。元写真のおおまかな配置、物同士の関係、中心動作、少なくとも3つの固有手掛かりを保ち、丸く少し誇張した形、粗く揺れて所々途切れる黒線、高明度・高純度の平塗りへ再構成します。
...
**Panel 017** は、原写真の核を保ちながら、次の視覚方向を目指す制作に向いています：**写真を、明るく親しみのある韓国風の素朴なフラット編集画へ**。まず用途と期待する結果を確認し、そこから構図、サイズ、文字、納品形式を決めます。
...
> XXD Panel 017 でこの画像を処理して、まず最適な構図とサイズを提案して。
...
## すぐに判断：XXD Panel 017 はあなたに合う？

| 知りたいこと | このスタイルの答え |
|---|---|
| **得られる完成作品** | 写真を、明るく親しみのある韓国風の素朴なフラット編集画へ |
| **ひと目で分かる特徴** | 丸い形 · 粗く途切れる線 · 純色の平塗り · 明るい色面 · 軽快な非対称 |
...
```text
配置・関係・動作・3つの固有手掛かりを固定 → 丸く少し誇張して簡略化 → 粗く揺れ局所的に途切れる黒線 → 高明度・高純度の平塗り → 大小差・ずれ・遮蔽・軽い裁ち落とし・余白で中心を一つ作る → タイトルと小文字を輪郭・色面・余白・軸へ組み込む
```

無関係な写真に替えても配置、関係、動作、形、色面、文字がほぼ成立するなら、それは 017 ではありません。

## 完成作品で見分けやすいスタイル特性

- **元写真の個性：** おおまかな配置、物同士の関係、中心動作、少なくとも3つの固有手掛かりを守ります。
- **丸い簡略化：** 形は単純で少し誇張し、写実的な光、遠近、材質を除きながら一目で識別可能にします。
- **粗く途切れる線：** 黒い輪郭は揺れ、太さが少し変わり、局所的に自然に切れます。滑らかなベクターや均一な漫画線ではありません。
- **純色の平塗り：** 不透明で清潔な色面を使い、奥行きは平面の重なり、大きさ、位置だけで表します。
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 017 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-017 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-017 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-017 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-017 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 017 を適用し、共通設定は一度だけ解決
/xxd-panel-017 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを `~/.claude/skills/xxd-panel-017` にリンクできます。インストール後に Agent セッションを再起動してください。
...
- [中国語ランタイムアダプター](references/xxd-panel-017-prompt.zh-CN.md)
- [英語ランタイムアダプター](references/xxd-panel-017-prompt.en.md)
...
**写真をただ可愛くせず、少ない形、明るい色、自由な途切れ線で、その写真らしさを見つける。**
