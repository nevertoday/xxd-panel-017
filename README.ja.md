...
| モード | サイズ方針 | 成果物 |
| --- | --- | --- |
| `top-bottom` | 元画像適応 | 上に完全な元写真、下に 017 の韓国風素朴フラット編集画。両パネルは元画像サイズを保ち、厳密に 50/50 |
| `left-right` | 元画像適応 | 左に完全な元写真、右に 017 の韓国風素朴フラット編集画。両パネルは元画像サイズを保ち、厳密に 50/50 |
| `design-only` | 元画像適応 | 元写真を見せず、変化デザインだけを表示。元画像の比率と寸法を維持 |
| `wallpaper-pack` | 端末別4サイズ | スマートフォン、iPad、デスクトップ、子ども用ウォッチの PNG を個別出力 |

優先順位は、正確な指定ピクセル > 指定比率／用途 > 通常モードの元画像適応です。原稿 `017.md` の 3:4 は初期の制作画角であり、現在の暗黙の既定値ではありません。
...
- **独立4作品：** 各端末は元写真だけを参照し、丸い形、途切れる線、明るい色面、余白、文字の関係をそれぞれ探れます。
...
自動文案は、主体、見える動作、関係、確認済みの場所、感情の手掛かり、または写真に根拠のある物語関係から短いタイトルを抽出します。この写真と強く結びつく小さな納得感を作りつつ、物語や語呂合わせを捏造しません。
...
画像モデルが丸い形、粗く途切れる線、純色の平塗り、明るい色面、軽快な非対称、余白、文字を作ります。`scripts/compose_panel.py` は画布計画、厳密な 50/50 ラスター合成、最終寸法、監査だけを担当し、プログラム描画で作品を偽装しません。
...
- [中国語の完全プロンプト](references/xxd-panel-017-prompt.zh-CN.md)
- [英語の完全プロンプト](references/xxd-panel-017-prompt.en.md)
